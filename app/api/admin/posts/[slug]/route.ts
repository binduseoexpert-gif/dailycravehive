...
function filePath(slug: string) { return `${POSTS_DIR}/${slug}.${POSTS_EXT}`; }
...
export async function PUT(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    if (allImages.length > 0) {
      ...
      if (!imgResult.ok) return NextResponse.json({ error: imgResult.error }, { status: 500 });
    }
...
export async function DELETE(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const delRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/${filePath(slug)}`, {
      method: "DELETE",
      headers: { ...gh, "Content-Type": "application/json" },
      body: JSON.stringify({ message: `post: delete "${slug}"`, sha: file.sha, branch: GITHUB_BRANCH }),
    });
    if (!delRes.ok) {
      const err = await delRes.json().catch(() => ({}));
      return NextResponse.json({ error: `GitHub delete failed: ${err.message || delRes.statusText}` }, { status: 500 });
    }
    return NextResponse.json({ ok: true });
  } catch {
    return NextResponse.json({ error: "Server error." }, { status: 500 });
