  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      <div className="mb-8 flex items-start justify-between gap-4">
        ...
        <a href="/admin/posts" className="shrink-0 rounded-lg border border-neutral-300 px-4 py-2 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50">
          Manage posts →
        </a>
      </div>
      ...
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            {POST_TYPES.map((t) => (
              <button key={t.id} type="button" onClick={() => setPostType(t.id)}
                className={`rounded-lg border px-3 py-2.5 text-left text-sm transition ${postType === t.id ? "border-amber-500 bg-amber-50 font-semibold text-amber-800" : "border-neutral-300 bg-white text-neutral-700 hover:border-neutral-400"}`}>
                <div>{t.label}</div>
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input className={input} value={slug} onChange={(e) => { setSlugEdited(true); setSlug(slugify(e.target.value)); }} placeholder="notion-ai-review" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            ...
            {postType === "review" ? (
              ...
              <input className={input} value={blogCategory} onChange={(e) => setBlogCategory(e.target.value)} placeholder='e.g. "Lifestyle", "AI News", "Guides"' />
            ) : (
          ...
          <div>
            ...
            {imagePreview ? (
              <div className="mt-2 flex items-center gap-3">
                ...
                <span className="text-xs text-neutral-500">/images/{slug || "slug"}.{imageExt} (auto)</span>
                <button type="button" onClick={clearImage} className="text-xs text-red-600 underline">Delete</button>
              </div>
            ) : (
              <input className={`${input} mt-2`} value={thumbnail} onChange={(e) => setThumbnail(e.target.value)} placeholder={`or manual path: /images/${slug || "post-slug"}.png`} />
            )}
          </div>
        </div>
        ...
        <div>
          ...
          <textarea className={input} rows={3} value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder={"best AI video tools\nAI video tools 2026\nbest AI video generator"} />
        </div>
          ...
          <div className="flex flex-wrap items-end gap-3">
            {postType === "review" && (
              <div className="grow">
                ...
                <input className={input} value={toolName} onChange={(e) => setToolName(e.target.value)} placeholder="e.g. Notion AI" />
              </div>
            )}
            {postType === "comparison" && (
              <>
            ...
            <button type="button" onClick={loadTemplate} className="rounded-lg border border-amber-400 bg-white px-4 py-2 text-sm font-medium text-amber-700 transition hover:bg-amber-100">
              Load template
            </button>
          </div>
        </div>
        ...
        <button onClick={handleSubmit} disabled={status.type === "loading"}
        ...
        {status.type === "success" && (
          <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
            Post committed! It will be live after the deploy finishes: <a className="font-medium underline" href={status.url} target="_blank" rel="noreferrer">{status.url}</a>
          </div>
        )}
        {status.type === "error" && (
